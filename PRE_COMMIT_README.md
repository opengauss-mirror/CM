# CM pre-commit Developer Guide

## What Was Ported From DSS

[DSS PR #700](https://gitcode.com/opengauss/DSS/pull/700) added a local pre-commit workflow (originally adapted from `mcp-opengauss`):
...
CM is a C/C++ repository with CMake, shell scripts, and some Python tooling under `tool/`. The Python-only checks in MCP (`ruff`, `darker`, `mypy`, `bandit`) are not copied directly. This setup keeps the same operating model and uses checks that fit this repository.

## Checks Enabled For CM
...
```bash
.pre-commit-venv/bin/pre-commit run --files src/cm_server/cms_main.cpp
...
- The hook repositories point to gitcode mirrors, matching the MCP/DSS setup for environments without direct github access.
- `.clang-format` is copied from `openGauss-server` so `clang-format-diff` follows the same C/C++ style as other openGauss components.
