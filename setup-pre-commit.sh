#!/usr/bin/env bash
# =============================================================================
# setup-pre-commit.sh -- install CM pre-commit checks in an isolated venv.
#
# Design:
...
git rev-parse --show-toplevel >/dev/null 2>&1 || die "run this script inside the CM git repository"
...
log "manual run: $PRE_COMMIT run --files <path>"
