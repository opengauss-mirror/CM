...
static bool IsXalarmLateAckEvent(int alarmId)
{
    return (alarmId == (int)XALARM_REBOOT_EVENT_ID);
}
 ...
 * reboot/OOM: msgid_res; panic/kernel reboot: msgid_{cna,eid}_res; res=0 means success.
 */
...
static void ProcessXalarmFaultFlow(struct alarm_info *param, int alarmId, uint32 alarmNodeId, uint32 cna)
{
    ...
    if (alarmDesc == NULL) {
        ...
        if (IsXalarmLateAckEvent(alarmId)) {
            ReportXalarmEventAck(alarmId, NULL, XALARM_ACK_RES_FAIL);
        }
        return;
    }
...
static void HandleXalarmPowerOffEvent(struct alarm_info *param)
{
    if (g_currentNode == NULL) {
        write_runlog(ERROR, "xalarm power off event but current node is null.\n");
        ReportXalarmEventAck((int)XALARM_REBOOT_EVENT_ID, xalarm_getdesc(param), XALARM_ACK_RES_FAIL);
        return;
    }

    write_runlog(LOG, "receive xalarm power off event, nodeId=%u, alarmType=%d, alarmTime=%lld.\n",
        g_currentNode->node, xalarm_gettype(param), (long long)xalarm_gettime(param));
    ProcessXalarmFaultFlow(param, (int)XALARM_REBOOT_EVENT_ID, g_currentNode->node, 0);
}
...
static void HandleXalarmEvent(struct alarm_info *param)
{
    ...
    if (alarmId == (int)XALARM_REBOOT_EVENT_ID) {
        HandleXalarmPowerOffEvent(param);
        return;
    }
...
static void *XalarmEventCheckMain(void *arg)
{
    ...
    uint32 filterIdx = 0;
    idFilter.id_list[filterIdx++] = (int)XALARM_REBOOT_EVENT_ID;
