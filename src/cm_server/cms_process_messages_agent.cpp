...
static void GetMasterResInstanceId(CmResStatList *resList)
{
    vector<string> tmp;

    for (uint32 i = 0; i < CM_MAX_RES_NODE_COUNT; ++i) {
        for (uint32 j = 0; j < resList->nodeStatus[i].count; ++j) {
            string str(resList->nodeStatus[i].status[j].resName, strlen(resList->nodeStatus[i].status[j].resName));

            resList->nodeStatus[i].status[j].isMaster = true;
            for (string it : tmp) {
                resList->nodeStatus[i].status[j].isMaster = !(it == str);
            }

            if (resList->nodeStatus[i].status[j].isMaster &&
                resList->nodeStatus[i].status[j].status == CM_RES_STAT_ONLINE) {
                tmp.push_back(str);
                continue;
            }
            resList->nodeStatus[i].status[j].isMaster = false;
        }
    }
}

static void CopyResListToMsg(CmResStatList *resList)
{
    errno_t rc;
    uint64 &version = GetResStatusListVersion();
    vector<OneNodeResStatusInfo> &resStatusVector = GetResStatus();

    if (resStatusVector.empty()) {
        return;
    }

    resList->version = version;
    (void)pthread_rwlock_init(&resList->lock, NULL);

    write_runlog(DEBUG1, "copy res status to send msg.\n");
    for (uint64 i = 0; i < static_cast<uint64>(resStatusVector.size()); ++i) {
        (void)pthread_rwlock_wrlock(&(resStatusVector[i].lk_lock));
        rc = memcpy_s(&resList->nodeStatus[i], sizeof(OneNodeResourceStatus),
                      &resStatusVector[i].nodeStatus, sizeof(OneNodeResourceStatus));
        securec_check_errno(rc, (void)rc);
        (void)pthread_rwlock_unlock(&(resStatusVector[i].lk_lock));
    }

    GetMasterResInstanceId(resList);
}

static void ProcessReportResChangedMsg(bool notifyClient)
{
    CmsReportResStatList sendMsg;

    if (notifyClient) {
        sendMsg.msgType = MSG_CM_AGENT_RES_STATUS_CHANGED;
    } else {
        sendMsg.msgType = MSG_CM_AGENT_RES_STATUS_LIST;
    }
    CopyResListToMsg(&sendMsg.resList);

    (void)pthread_rwlock_wrlock(&gConns.lock);
    for (uint32 i = 0; i <= gConns.count; ++i) {
        cm_server_send_msg(gConns.connections[i], 'S', (char*)(&sendMsg), sizeof(CmsReportResStatList));
    }
    (void)pthread_rwlock_unlock(&gConns.lock);
}

static void ChangeInstanceData(OneNodeResourceStatus &oldStatus, const OneNodeResourceStatus &newStatus)
{
    errno_t rc;

    if (oldStatus.count != newStatus.count) {
        rc = memcpy_s(&oldStatus, sizeof(OneNodeResourceStatus), &newStatus, sizeof(OneNodeResourceStatus));
        securec_check_errno(rc, (void)rc);
        return;
    }

    for (uint32 i = 0; i < oldStatus.count; ++i) {
        if (oldStatus.status[i].status != newStatus.status[i].status) {
            if (oldStatus.status[i].status == (uint32)CM_RES_STAT_ONLINE &&
                newStatus.status[i].status == (uint32)CM_RES_STAT_OFFLINE) {
                oldStatus.status[i].instanceData = 0;
            }
            oldStatus.status[i].status = newStatus.status[i].status;
        }
    }
}
...
{
    if (g_res_list.empty()) {
        write_runlog(LOG, "(client) no resource, continue processing is unnecessary, ProcessAgentToCmResource.\n");
        return;
    }
    uint64 &version = GetResStatusListVersion();
    vector<OneNodeResStatusInfo> &resStatusVector = GetResStatus();
    uint64 nodeId = (uint64)resourceStatusPtr->node_status.node;

    if (nodeId > CM_MAX_RES_NODE_COUNT) {
        write_runlog(ERROR, "nodeId(%lu) is bigger than 16, can't process this msg.\n", nodeId);
        return;
    }

    ++version;
    write_runlog(LOG, "nodeId = %u, status has changed.\n", resourceStatusPtr->node_status.node);
    (void)pthread_rwlock_wrlock(&(resStatusVector[nodeId - 1].lk_lock));
    ChangeInstanceData(resStatusVector[nodeId - 1].nodeStatus, resourceStatusPtr->node_status);
    (void)pthread_rwlock_unlock(&(resStatusVector[nodeId - 1].lk_lock));

    ProcessReportResChangedMsg(resourceStatusPtr->isStatusChanged);
}
...
{
    if (g_res_list.empty()) {
        write_runlog(LOG, "(client) no resource, continue processing is unnecessary, ProcessRequestResStatusList.\n");
        return;
    }
    CmsReportResStatList sendMsg;
    ...
    cm_server_send_msg(con, 'S', (char*)(&sendMsg), sizeof(CmsReportResStatList));
}

void ProcessSetInstanceDataMsg(const ReportSetInstanceData *recvMsg)
{
    if (g_res_list.empty()) {
        write_runlog(LOG, "(client) no resource, continue processing is unnecessary, ProcessSetInstanceDataMsg.\n");
        return;
    }
    CmsReportResStatList sendMsg;
    uint64 &version = GetResStatusListVersion();
    ...
    if (recvMsg->nodeId > CM_MAX_RES_NODE_COUNT) {
        write_runlog(LOG, "(client) nodeId=%u is bigger than 16.\n", recvMsg->nodeId);
        return;
    }
    ++version;
    for (uint64 i = 0; i < static_cast<uint64>(resStatusVector[recvMsg->nodeId - 1].nodeStatus.count); ++i) {
        if (resStatusVector[recvMsg->nodeId - 1].nodeStatus.status[i].instanceId == recvMsg->instanceId) {
            resStatusVector[recvMsg->nodeId - 1].nodeStatus.status[i].instanceData = recvMsg->data;
            break;
        }
    }

    write_runlog(LOG, "(client) InstanceId=%d set InstanceData=%ld success\n", recvMsg->instanceId, recvMsg->data);
...
{
    if (g_res_list.empty()) {
        write_runlog(LOG, "(client) no resource, continue processing is unnecessary, ProcessReportSetResDataMsg.\n");
        return;
    }
    recvMsg->resName[CM_MAX_RES_NAME - 1] = '\0';
    recvMsg->resData.data[CM_MAX_RES_DATA_SIZE - 1] = '\0';
    uint64 i;
    errno_t rc;
    CmsReportSetDataResult sendMsg;
    ...
    if (resDataList[i].version == recvMsg->resData.version) {
        write_runlog(LOG, "client saved version is same with reality version.\n");
    }

    resDataList[i].version++;
    uint32 len = recvMsg->resData.size;
    if (recvMsg->resData.size >= CM_MAX_RES_DATA_SIZE) {
    ...
    rc = memcpy_s(resDataList[i].resData[recvMsg->resData.slotId].data, CM_MAX_RES_DATA_SIZE,
        recvMsg->resData.data, len);
    securec_check_errno(rc, (void)rc);

    write_runlog(LOG, "(client) set resData success, slotId(%ld)\n", recvMsg->resData.slotId);

    sendMsg.msgType = (int)MSG_CM_AGENT_REPORT_SET_STATUS;
    sendMsg.conId = recvMsg->conId;
    sendMsg.isSetSuccess = true;

    cm_server_send_msg(con, 'S', (char*)(&sendMsg), sizeof(CmsReportSetDataResult));
}
...
void ProcessRequestGetResData(CM_Connection *con, RequestGetResData *recvMsg)
{
    ...
    sendMsg.msgType = MSG_CM_AGENT_REPORT_RES_DATA;
    sendMsg.resData.version = resDataList[i].version;
    if (recvMsg->slotId >= CM_MAX_RES_SLOT_COUNT) {
        write_runlog(ERROR, "ProcessRequestGetResData, slotId(%lu) is bigger than 8.\n", recvMsg->slotId);
        return;
    }
    sendMsg.resData.size = resDataList[i].resData[recvMsg->slotId].size;
    sendMsg.conId = recvMsg->conId;
    sendMsg.resData.slotId = recvMsg->slotId;
    rc = memcpy_s(
        sendMsg.resData.data, CM_MAX_RES_DATA_SIZE, resDataList[i].resData[recvMsg->slotId].data, sendMsg.resData.size);
    securec_check_errno(rc, (void)rc);

    write_runlog(LOG, "(client) send resData to client, slotId=%ld.\n", recvMsg->slotId);
    cm_server_send_msg(con, 'S', (char *)(&sendMsg), sizeof(CmsReportResData));
}
